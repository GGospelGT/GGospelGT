...
agent_communication:
  ...
  - agent: "testing"
    message: "❌ CRITICAL PORTFOLIO FRONTEND ISSUE: Portfolio system frontend has authentication and role detection problems. While all components are properly implemented (ImageUpload, PortfolioGallery, ProfilePage integration), the Portfolio tab is not showing for tradesperson users. Root cause: Authentication context not persisting properly on profile page, showing 'Sign In Required' even after successful login. This prevents testing of portfolio upload, gallery management, and public portfolio features. URGENT: Main agent needs to fix authentication state persistence and role detection in ProfilePage component."