#!/usr/bin/env python3
"""
Enhanced Job Posting Backend Testing - Final Version
...
BACKEND_URL = "https://servicepro-ng.preview.emergentagent.com/api"
...
class EnhancedJobPostingTester:
    ...
    def test_authentication_setup(self):
        """Set up authentication for testing by registering a new homeowner"""
        ...
        if response.status_code == 200:
            ...
            homeowner_token = homeowner_profile.get('access_token')
            if homeowner_token:
                self.log_result("Homeowner registration for enhanced job test", True, f"User: {homeowner_profile['user']['name']}")
                self.auth_tokens['homeowner'] = homeowner_token
                self.test_data['homeowner_user'] = homeowner_profile['user']
                return True
            else:
                self.log_result("Homeowner registration for enhanced job test", False, "No access token")
        else:
            self.log_result("Homeowner registration for enhanced job test", False, f"Status: {response.status_code}, Response: {response.text}")
    ...
    def test_enhanced_job_creation_authenticated(self):
        ...
        enhanced_job_data = {
            ...
            # Include homeowner details (even though user is authenticated)
        ...
        if response.status_code == 200:
            ...
            if not missing_fields:
                ...
                if homeowner.get('email') == homeowner_user['email']:
    ...
    def test_location_field_validation(self):
        ...
        missing_fields_job = {
            "title": "Test Job - Missing Fields",
            "description": "This is a test job with missing required fields to test validation.",
            "category": "Carpentry",
            "state": "Lagos",
            "lga": "Surulere",
            # Missing: town, zip_code, home_address
            "budget_min": 100000,
            "budget_max": 200000,
        ...
        response = self.make_request("POST", "/jobs/", json=missing_fields_job, auth_token=homeowner_token)
        if response.status_code in [400, 422]:
            self.log_result("Missing required fields validation", True, "Correctly rejected job with missing fields")
        else:
            self.log_result("Missing required fields validation", False, f"Expected 400/422, got {response.status_code}")
        
        # Test 4: Valid enhanced job creation
        valid_enhanced_job = {
    ...
    def test_error_handling(self):
        """Test comprehensive error handling for enhanced job posting"""
        print("\n=== Testing Enhanced Job Error Handling ===")
        
        if 'homeowner' not in self.auth_tokens:
            self.log_result("Enhanced job error handling tests", False, "No homeowner token available")
            return
        ...
        short_title_job = {
            ...
            "description": "This is a test job with a title that is too short to meet the minimum length requirement.",
            "category": "Plumbing",
            "state": "Lagos",
            "lga": "Ikeja",
            "town": "Ikeja GRA",
            "zip_code": "100001",
            "home_address": "Test address for short title validation",
            "budget_min": 50000,
            "budget_max": 100000,
            "timeline": "Within 2 weeks",
            "homeowner_name": homeowner_user['name'],
        ...
        response = self.make_request("POST", "/jobs/", json=short_title_job, auth_token=homeowner_token)
        if response.status_code in [400, 422]:
            self.log_result("Title length validation", True, "Correctly rejected short title")
        else:
            self.log_result("Title length validation", False, f"Expected 400/422, got {response.status_code}")
        
        # Test 2: Description too short
        short_desc_job = {
            "title": "Test Job - Short Description Validation",
            ...
            "town": "Surulere",
            "zip_code": "101283",
            "home_address": "Test address for short description validation",
            "budget_min": 75000,
            "budget_max": 150000,
            "timeline": "Within 3 weeks",
            "homeowner_name": homeowner_user['name'],
        ...
        response = self.make_request("POST", "/jobs/", json=short_desc_job, auth_token=homeowner_token)
        if response.status_code in [400, 422]:
            self.log_result("Description length validation", True, "Correctly rejected short description")
        else:
            self.log_result("Description length validation", False, f"Expected 400/422, got {response.status_code}")
        
        # Test 3: Negative budget
        negative_budget_job = {
            "title": "Test Job - Negative Budget Validation",
            "description": "This is a test job with negative budget values to test validation of budget constraints.",
            "category": "Carpentry",
            "state": "Lagos",
            "lga": "Apapa",
            "town": "Apapa",
            "zip_code": "101007",
            "home_address": "Test address for negative budget validation",
            "budget_min": -50000,  # Negative budget
            "budget_max": 100000,
            "timeline": "Within 4 weeks",
            "homeowner_name": homeowner_user['name'],
            "homeowner_email": homeowner_user['email'],
        ...
        response = self.make_request("POST", "/jobs/", json=negative_budget_job, auth_token=homeowner_token)
        if response.status_code in [400, 422]:
    ...
    def test_database_integration(self):
        ...
        if response.status_code == 200:
            ...
            self.log_result("Database persistence of enhanced fields", False, 
                           f"Failed to retrieve job: {response.status_code}")
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            # Step 7: Test enhanced job creation without authentication
